...
from pytorch_lightning.callbacks import Callback, ModelCheckpoint, EarlyStopping # type: ignore
import torch # type: ignore
import torch.nn as nn # type: ignore
import torch.optim as optim # type: ignore
from torch.utils.data import DataLoader, Dataset, ConcatDataset, random_split # type: ignore
...
class ImageDataset(Dataset):
    ...
    # (Converts the input image to a PIL image if it is not already one.
    # Ensures the image is in RGB format.
    # Applies a series of transformations to the image and returns it as a PyTorch tensor.)

# this class can be used for training multiple models (not me, cursor, lol )
...
class EfficientNetDetector(pl.LightningModule):
    """EfficientNet-based model for real/fake image detection."""
    def __init__(self, config: ModelConfig):
        super().__init__() #to call the constructor of the parent class (or superclass) from within a subclass
        self.config = config
    ...
    def _build_model(self) -> None:
        ...
        self.base_model.eval() #model eval mode for the trained model
        for param in self.base_model.parameters():
            param.requires_grad = False

        # (Unfreeze the last few layers....the last 3 layers)
        # for param in self.base_model.get_classifier().parameters():
        #     param.requires_grad = True  # Unfreeze the classifier layer

        num_features = self.base_model.get_classifier().in_features #no of input features for the classifier
        self.base_model.classifier = nn.Identity() # repalce org classify with an identity layer
